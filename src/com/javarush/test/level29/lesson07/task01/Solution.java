        ...
        new Thread() {
            ...
            public void run() {
                for (int i = 0; i < 100000; i++) {
                    messageQueue.add(String.valueOf(i)); // просто i, зачем тут --?
                }
            }
        }.start();
    }
        ...
        for (int i = 0; i < NUMBER_OF_THREADS; i++) {
            new Thread() {
                ...
                public void run() {
                    while (true) {
                        try {
                            messageQueue.drainTo(batch, MAX_BATCH_SIZE); //тут исправляем (перенести максимальное число элементов из очереди в указанную коллекцию, т.е.из messageQueue в batch)
                            persistData(batch);
                            batch.clear();
                            Thread.sleep(1);
                        } catch (Throwable e) {
