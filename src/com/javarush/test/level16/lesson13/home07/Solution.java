...
public class Solution
{
    ...
    public static class OnlineGame extends Thread
    {
        ...
        static
        {
            steps.add("Начало игры");  //0
            steps.add("Сбор ресурсов"); //1
            steps.add("Рост экономики"); //2
            steps.add("Убийство врагов"); //3
        }
        ...
        public void run()
        {
            gamer1.start();  // запускаем 3 потока
            gamer2.start();
            gamer3.start();

            while (!isWinnerFound) // run выполняется
            {
            }
    ...
    public static class Gamer extends Thread
    {
        ...
        {
            super(name);
            this.rating = rating; //количеством действий в секунду
        }
