...
public class Solution
{
    ...
    public static void printStocks(List<Stock> stocks, Date actualDate)
    {
        ...
        Format[] testFormats = {null, null, dateFormat, fileform};  // тут была ошибка
    ...
    public static void sort(List<Stock> list)
    {
        ...
        {
            public int compare(Stock stock1, Stock stock2) // -1 : "<", 0 : "=", 1 : ">", т.е. должен быть int
            {
                int compareNames = ((String) stock1.get("name")).compareTo((String) stock2.get("name")); // сортируем по имени
                if (compareNames != 0)
                    return compareNames;
                else  // если имена одинаковые
                {
                    int compareDate = ((Date) stock1.get("date")).compareTo((Date) stock2.get("date")); // сортируем по дате
                    if (compareDate != 0)
                        return compareDate;
                    else // если даты одинаковые, возвращает 0
                    {
                        Double change1;
                        Double change2;
