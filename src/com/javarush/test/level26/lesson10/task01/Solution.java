...
public class Solution {
    ...
    public void someMethod() {
        ...
        if (lock.tryLock()) {  // попытаться захватить лок
            try {  // если лок свободен, то:
                ifLockIsFree();  // вызвать метод ifLockIsFree
            } finally { // отпустить лок при любых условиях, даже если ifLockIsFree будет кидать исключение
                lock.unlock();
            }
        } else {  // если лок занят, то вызвать метод ifLockIsBusy
            ifLockIsBusy();
        }
    }
