...
public class Solution {
    ...
    public static class AmigoThreadFactory implements ThreadFactory{
        ...
        private AtomicInteger factoryNum = new AtomicInteger(0); // это номер фабрики
        private AtomicInteger threadNum = new AtomicInteger(0); // номер треда

        public AmigoThreadFactory() {
           factoryNum.set(factoryCount.incrementAndGet()); // вот именно так нужно устанавливать это номер фабрики, используя статик factoryCount
        }
        ...
        public Thread newThread(Runnable r) {
            ...
            String threadName = thread.getThreadGroup().getName()
                    + "-pool-"
                    + factoryNum
                    + "-thread-"
                    + threadNum.incrementAndGet(); // все равно, что ++threadNum, только атомарно
